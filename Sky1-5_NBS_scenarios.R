...
endtime = 2100 # What year do you want to end the simulation?
...
for(scen in scenarios) {
  ...
  if(scen == "islands") {
    ...
    # Set the simulation time parameters for the second half of the islands scenario
    starttime = islandstime
    
    # Input the second half of the extent data and wrangle the adoption data to match this new timeframe
    extent_in = read.csv(file.path(Scenario_data, paste0("extent_islands2050.csv")), header=T) # Read in the inputs required to convert adoption rates to absolute values of hectares and emissions reductions
    adopt_in$year_50 = (adopt_in$year_50 - adopt_in$year_start) + starttime # Shift all 50% years up to account for the new specified 'start' date 
    adopt_in$year_start = starttime # Ensure all pathways are starting at 2050
    NBS_in = merge(adopt_in, extent_in) # Create one dataframe that can be used as inputs to the function
  ...
  } else { # If not the islands scenario it's much easier...
...
# Quick plotcheck
ggplot(all_scenarios, aes(x=year, y=TERs_MtCO2, fill=NBS_short_name)) +
  geom_area() +
  facet_wrap(~run_ID)
